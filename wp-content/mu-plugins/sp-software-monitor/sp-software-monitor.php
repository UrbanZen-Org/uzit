...
class Synthesis_Software_Monitor {
    ...
    public static function admin_menu() {
        if ( ! get_option( 'sng_level' ) ) {
            ...
            $page = add_menu_page( __( 'StudioPress Plugins' ), __( $menu_title ), 'install_plugins', 'sp-software-monitor', array( __CLASS__, 'software_monitor_page' ), $svg_icon, 2 );

            add_submenu_page( 'sp-software-monitor', __( 'StudioPress Plugins' ), __( 'Site Tools' ), 'install_plugins', 'sp-software-monitor', array( __CLASS__, 'software_monitor_page' ) );
    ...
    public static function sp_has_ssl( $domain ) {
        $response = wp_remote_head( 'https://' . $domain, array( 'timeout' => 2 ) );
